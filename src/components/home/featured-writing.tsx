...
function FeaturedPrimaryStory({ piece }: { piece: CardPiece }) {
  ...
  return (
    <motion.article
      {...cardMotionProps}
      className="group relative overflow-hidden rounded-xl border border-rule/60 bg-surface-raised/20 p-6 sm:p-8 lg:p-10 backdrop-blur-sm transition-all duration-300 hover:border-rule hover:bg-surface-raised/30"
    >
      <div className="grid gap-8 lg:grid-cols-[1.1fr_1fr] lg:items-center">
        {/* Left: 16:10 Cinematic Cover Image */}
        <Link href={href} className="block overflow-hidden rounded-lg bg-surface-raised/40">
          {piece.coverImage && (
        ...
        <div className="flex flex-col justify-center">
          <div className="flex items-center gap-2 text-xs font-mono tracking-widest text-accent">
            <span className="uppercase font-semibold">
              {meta ? (isBn ? meta.labelBn : meta.labelEn) : "FEATURED"}
            </span>
          </div>

          <h3
            className="mt-3 font-bengali text-2xl sm:text-3xl lg:text-4xl font-medium leading-tight text-content tracking-tight transition-colors duration-200 group-hover:text-accent"
            lang="bn"
          >
            <Link href={href}>
              {piece.titleBn}
          ...
          {piece.dekBn && (
            <p
              className="mt-4 font-bengali text-sm sm:text-base leading-relaxed text-content-soft line-clamp-3"
              lang="bn"
            >
              {piece.dekBn}
            </p>
          ...
          <div className="mt-6 pt-2">
            <Link
              href={href}
              className="inline-flex items-center gap-2 text-xs font-mono uppercase tracking-wider text-accent transition-transform duration-200 group-hover:translate-x-1 hover:opacity-80"
            >
...
function SecondaryArticleCard({ piece }: { piece: CardPiece }) {
  ...
  return (
    <motion.article
      {...cardMotionProps}
      className="group flex flex-col justify-between rounded-xl border border-rule/50 bg-gradient-to-b from-surface-raised/25 to-surface-raised/10 p-4 backdrop-blur-sm transition-all duration-300 hover:border-accent/40 hover:bg-surface-raised/35"
    >
      <Link href={href} className="block flex-1 flex flex-col">
        {piece.coverImage && (
          <div className="relative aspect-[16/10] w-full overflow-hidden rounded-lg bg-surface-raised/40">
            <CoverImageFrame
              owner="piece"
              slug={piece.slug}
              coverImage={piece.coverImage}
        ...
        <div className="mt-4 flex-1 flex flex-col justify-between">
          <div>
            ...
            <h4
              className="mt-2.5 font-bengali text-lg font-medium leading-snug text-content tracking-tight transition-colors duration-200 group-hover:text-accent line-clamp-2"
              lang="bn"
            >
              {piece.titleBn}
            </h4>
      ...
      <div className="mt-4 pt-3 border-t border-rule/30 flex items-center justify-between">
        <Link
          href={href}
          className="inline-flex items-center gap-1 text-xs font-medium text-accent transition-transform duration-200 group-hover:translate-x-0.5 hover:opacity-80"
        >
